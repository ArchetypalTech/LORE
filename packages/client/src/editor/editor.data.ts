...
type AnyObject = T_Action | T_Room | T_Object | T_TextDefinition | Player;
...
const clearRoom = (id: string) => {
	set((prev) => ({
		...prev,
		rooms: {
			...Object.entries(get().rooms).filter((room) => room[1].roomId !== id),
		},
	}));
	clearItem(id);
};
...
const syncItem = (obj: unknown) => {
	if ("Player" in (obj as { Player: Player })) {
		const player = { ...(obj as { Player: Player }) }.Player;
		player.inst = player.inst.toString();
		setItem(player, player.inst);
	}
...
const getRoomForObject = (obj: T_Object) => {
	for (const room of Object.values(get().rooms) as T_Room[]) {
		if (room.objectIds.includes(obj.objectId)) return room;
	}
	return null;
};
...
const getObjectForAction = (action: T_Action) => {
	for (const object of Object.values(get().objects) as T_Object[]) {
		if (object.objectActionIds.includes(action.actionId)) return object;
	}
	return null;
};
...
const removeActionFromObject = async (action: T_Action) => {
	...
	object.objectActionIds = object.objectActionIds.filter(
