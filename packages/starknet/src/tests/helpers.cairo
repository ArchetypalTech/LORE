...
pub fn setup_core() -> HelperSystems {
    ...
    // messaging contracts
    let messaging_contract: ContractAddress = 0x0.try_into().unwrap();
    let appchain_contract: ContractAddress = 0x0.try_into().unwrap();
    ...
    world.dispatcher.grant_owner(lore_sn::lib::dns::SELECTORS::PERMIT_TOKEN.into(), OWNER());
