...
if (!(await isCommandAvailable("asdf"))) {
  if (process.platform !== "darwin" || !(await isCommandAvailable("brew"))) {
    console.log(
      `ℹ️ Please follow the installation instructions (https://asdf-vm.com/guide/getting-started.html) for installing ${bgDarkGray(
        " asdf "
  ...
  console.log(
    `ℹ️ Please review the post-installation instructions (https://asdf-vm.com/guide/getting-started.html#_2-configure-asdf) for setting up ${bgDarkGray(
      " asdf "
    )}`
  );
}
...
if (!versionSatisfies(await $`asdf --version`.text(), "0.16.0")) {
  console.log(
    `ℹ️ Please review the upgrade instructions (https://asdf-vm.com/guide/upgrading-to-v0-16.html) and reinstall ${bgDarkGray(
      " asdf "
    )} through ${
      process.platform === "darwin" ? "HomeBrew" : "your package manager"
    }.`
  );
  process.exit(0);
}
