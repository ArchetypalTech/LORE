...
Step-by-step plan for building Part 2 of [monetization-revenue-distribution.md](monetization-revenue-distribution.md) (owner/creator/collaborator revenue split on player actions). For how a credited address turns that credit into spendable actions, see [claiming-rewards.md](claiming-rewards.md); for an API-style reference across the whole spend/split/claim system, see [monetization-api.md](monetization-api.md).
